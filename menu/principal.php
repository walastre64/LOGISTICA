...
<div class="table-responsive">      
    ...
    <select id="cbo_horario" class="form-control form-select form-select-sm mb-3" aria-label="Selecione el horario">
      <option value="0" selected>- Horario -</option>
        ...
        <?PHP 
        $qry = "SET ANSI_NULLS ON SET ANSI_WARNINGS ON EXEC [dbo].[SP_BUS_HORARIO] 0,".$rol;
        $rst = sqlsrv_query( $conn, $qry, array(), array("Scrollable"=>"buffered"));
        if (! $rst) {  
           echo "Error en la ejecución de la instrucción ".$qry.".\n";
           die( print_r2( sqlsrv_errors(), true));  
...
<select id="cbo_reporte" style="flex: 1;" class="form-control form-select form-select-sm mb-3" aria-label="Selecione el horario">
                ...
                <option value="4">Rango Fecha</option>
                <option value="5">AM</option>
                <option value="6">PM</option>
                <option value="7">AMANECER</option>
...
const tabla1 = document.getElementById("tabla1");
	...
	$("#btn_buscar").click(function(){
		...
		if(seleccion == 0)
		{
			 toastr.error("Debe selecionar un tipo de -- <b> Reporte <b/> --", "Error !!!", {
			  "showDuration": "50",
			  "hideDuration": "500",
			  "timeOut": "2000",
		...
		if(seleccion == 4){
			  $.ajax({
				type:"GET",
				url: "reporte_fecha_ini_fin.php",		
				success: function() {	
				 //window.open ("reporte_fecha_ini_fin.php?txt_fecha_ini="+datepicker+"&txt_fecha_fin="+datepicker2, '_blank'); 			  
				 window.open ("reporte_fecha_ini_fin.php?txt_fecha_ini="+datepicker+"&txt_fecha_fin="+datepicker2+"&cbo_reporte="+seleccion, '_blank');
				}
			  });
		}else if(seleccion == 1){
		...
		}else if(seleccion == 5 || seleccion == 6 || seleccion == 7){
	...
	$("#btn_grabar").click(function(){
		...
		//alert(result);			
			...
			if(resultado[0] == 0){
				...
				"showDuration": "50","hideDuration": "500","timeOut": "2000","extendedTimeOut": "1000","preventDuplicates": true,"onclick": null,"progressBar": true,"positionClass": "toast-bottom-full-width"});				
				// redircionar a si mismo					
				setTimeout(function(){ location.href='principal.php';}, 1000);
				//return result;
...
(function($) {
  $.fn.replaceClass = function(classes) {
	...
	$("#btn_activa").click(function(){
		...
		var horario	 = document.getElementById('cbo_horario').value;
		
		if(horario == 0){
			toastr.error('Debe Selecionar el Horario que va a Cerrar', "Error !!!", {
			"showDuration": "50","hideDuration": "500","timeOut": "2000","extendedTimeOut": "1000","preventDuplicates": true,"onclick": null,"progressBar": true,"positionClass": "toast-bottom-full-width"});	
			return false;			
		...
		if(id_valor == 0)
		{
		  ...
		  // ejecuta qry desactiva lo seleccionado   [SP_ACT_HORARIO_LOGISTICA] 0 //
		 
			$.ajax({
			type: "POST",
			dataType:"html",
			url: "../datos/act_horario.php",
			data:"cbo_horario="+horario,
			cache: false,			
			success: function(result) {	
			resultado=result.split("/");
			//alert(result);			
			...
			if(resultado[0] == 1){
				toastr.success('Horario Actualizado !!!', "Exito !!!", {
				"showDuration": "50","hideDuration": "500","timeOut": "2000","extendedTimeOut": "1000","preventDuplicates": true,"onclick": null,"progressBar": true,"positionClass": "toast-bottom-full-width"});				
				// redircionar a si mismo					
				setTimeout(function(){ location.href='principal.php';}, 1000);
				//return result;
				...
				toastr.error('Error Actualizando los datos en  la tabla < [[M_HORARIO]] > !!!', "Error !!!", {
				"showDuration": "50","hideDuration": "500","timeOut": "2000","extendedTimeOut": "1000","preventDuplicates": true,"onclick": null,"progressBar": true,"positionClass": "toast-bottom-full-width"});	
				return false;			
			}		
		},			
		error: function(error) {				
			alert("Problemas con la pagina [SP_ACT_HORARIO_LOGISTICA] ... comuniquese con el Personal de Sistemas !!!" + error);
			}		
		});	
